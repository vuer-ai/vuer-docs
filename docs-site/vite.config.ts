...
export default defineConfig({resolve:{alias:{'@docs':fileURLToPath(new URL('.',import.meta.url))}},plugins:[catalogAliases(),...dockit()],define:{__DOCS_VERSION__:JSON.stringify(branch.startsWith('docs/')?branch.slice(5).replace(/^v/,''):manifest.current),__DOCS_BRANCH__:JSON.stringify(branch),__GIT_HASH__:JSON.stringify(execSync('git rev-parse --short HEAD').toString().trim())},server:{port:3035}})
