...
const sources=walk(content).filter(f=>f.endsWith('.mdx')),redirects=[],links=[]
for(const file of sources){
 const rel=path.relative(content,file).split(path.sep).join('/'),mdx=fs.readFileSync(file,'utf8')
...
for(const file of walk(out).filter(f=>f.endsWith('+Page.mdx'))){const rel=path.relative(out,path.dirname(file));write(path.join(pub,rel+'.md'),fs.readFileSync(file,'utf8'))}
