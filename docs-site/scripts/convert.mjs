...
const root=path.resolve(import.meta.dirname,'../..'), site=path.join(root,'docs-site'), source=path.join(root,'docs'), out=path.join(site,'pages'), pub=path.join(site,'public')
...
const files=walk(source), pages=files.filter(f=>f.endsWith('.md')&&!f.includes('/assets/')&&!f.includes('/_static/'))
...
function url(rel){return '/'+rel.replace(/\.md$/,'').replace(/(^|\/)index$/,'').replace(/\/$/,'')}
...
for(const f of pages){
 const rel=path.relative(source,f).split(path.sep).join('/'); let text=fs.readFileSync(f,'utf8').replaceAll('{VERSION}',version)
