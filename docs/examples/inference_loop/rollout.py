"""rollout -- a vuer example: a VLA policy rolling a trajectory in a splat scene.

This file generates rollout.md (cmx). The code blocks on the page are
explanatory excerpts from the live demo's viewer -- there is nothing to run
here. For the full, runnable demo (OmniVLA weights + GPU backend + viewer +
one-command deploy) see:

    https://github.com/vuer-examples/vuer-omnivla-rollout-demo
